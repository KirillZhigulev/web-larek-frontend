...
`_closeButton: HTMLButtonElement` - для отображения кнопки закрытия модального окна. Привязывает события закрытие модального окна `(modal:close)` к кликам по кнопке закрытия формы и по родительскому контейнеру модального окна
